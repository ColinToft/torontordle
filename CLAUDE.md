...
- `src/dailyCase.ts` — the deterministic **scheduler**, split frozen/live. `buildSchedule(cases, year, today, history)` reads frozen days from `history` verbatim, then computes the not-yet-frozen tail live (pool = unlocked minus used, resetting when exhausted; current week weighted ×1.5). Frozen entries resolve back to the bank by **diagnosis** (reorder-stable), falling back to sheet-row **id** (rename-stable) — a spelling fix to an answer self-heals; a removed case skips that one archive day. `freezeThrough` extends history append-only (used by the sync); `assertAppendOnly` enforces "a past day never changes". `caseForDate` = a single day; empty history degrades to a fully-live schedule (= the old behavior). Same for everyone. "Day NNN" badge from `LAUNCH_DATE_ET`.
